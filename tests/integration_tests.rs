...
async fn test_htmx_vehicles_returns_fragment() {
    ...
    let response = env.server.get("/htmx/vehicles").with_auth(&user.token).await;
...
async fn test_import_preview_accepts_csv() {
    ...
    let vehicle_id = common::create_test_vehicle_db(&env.pool, user.id, "Import", "Car", "IMP-1").await;
